...
public class MainActivity extends AppCompatActivity {
    ...
    private BroadcastReceiver myBroadcasReceier;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        myBroadcasReceier = new BroadcastReceiver() {
            @Override
            public void onReceive(Context context, Intent intent) {
                Bundle myBundle = intent.getExtras();
                int myInt = myBundle.getInt("background_service");

                run_time.setText("後台Servvice執行了"+myInt+"秒");
            }
        };
        IntentFilter intentFilter= new IntentFilter("MyMessage");

        registerReceiver(myBroadcasReceier,intentFilter);
    }
}
